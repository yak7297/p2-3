...
- 속도·비용: Flash Lite 모델, 입력 1,000자·출력 500토큰 제한을 사용한다. 향후 해시 기반 단기 캐시, 사용자별 호출 제한, `429` 처리와 트래픽 측정을 적용한다.
- 확장 구조: 기능이 늘어나면 프론트를 요청·폼·결과 모듈로, 백엔드를 API·검증·AI provider 모듈로 분리하고 `/api/v1/` 버전을 도입한다.
- 키 유출 대응: 노출이 의심되면 기존 키를 즉시 폐기·재발급하고 Vercel 환경 변수를 교체한 뒤 재배포한다. 커밋 이력을 검사하고 GitHub Secret Scanning 및 CI 비밀정보 검사를 적용한다.
- 프레임워크 검토: 이번 과제 제약에 따라 바닐라 JavaScript를 유지한다. 제약이 없는 후속 버전에서 React를 도입하면 컴포넌트 재사용성이 좋아지지만 빌드 설정·의존성·번들 복잡도가 증가한다.

캐싱·쿼터·비동기 처리, API 버전화, 키 사고 대응 절차와 프레임워크 도입 시 파일 변경 범위는 [서비스 기획서](docs/service-plan.md)의 10~13절에 정리했습니다.
